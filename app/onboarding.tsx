...
function ReportingBlock() {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: '#FFFFFF', paddingTop: 8, paddingBottom: 20 }}>
      ...
      {/* 3 stat tiles */}
      <View style={{ flexDirection: 'row' as any, paddingHorizontal: 24, gap: 10, alignItems: 'flex-end' as any }}>
        {STAT_TILES.map((tile, i) => {
          ...
          return (
            <TouchableOpacity
              ...
              style={{
                flex: 1,
                height: active ? 148 : 116,
                borderRadius: 20,
                backgroundColor: active ? tile.color : '#F7F7F7',
                borderWidth: active ? 0 : 1,
                borderColor: '#EBEBEB',
                alignItems: 'center' as any,
                justifyContent: 'center' as any,
                gap: 6,
                shadowColor: active ? tile.color : '#000',
                shadowOffset: { width: 0, height: active ? 12 : 2 },
                shadowOpacity: active ? 0.30 : 0.04,
                shadowRadius: active ? 20 : 6,
                elevation: active ? 8 : 1,
                ...(Platform.OS === 'web' ? { transition: 'all 0.45s cubic-bezier(0.34,1.56,0.64,1)' } as any : {}),
              }}
            >
              <Text style={{ fontSize: active ? 28 : 22 }}>{tile.icon}</Text>
              <Text style={{
                fontSize: active ? 22 : 17, fontWeight: '900' as any,
                color: active ? '#FFFFFF' : '#0D1117', letterSpacing: -0.5,
                ...(Platform.OS === 'web' ? { transition: 'all 0.35s ease' } as any : {}),
              }}>{tile.stat}</Text>
              <Text style={{
                fontSize: 9, fontWeight: '600' as any,
              ...
              {active && (
                <View style={{
                  position: 'absolute' as any, bottom: 12,
                  paddingHorizontal: 8, paddingVertical: 3,
                  borderRadius: 6, backgroundColor: 'rgba(255,255,255,0.22)',
                }}>
                  <Text style={{ fontSize: 7, fontWeight: '900' as any, color: '#FFFFFF', letterSpacing: 1 }}>
                    {tile.label}
                  </Text>
                </View>
              )}
      ...
      {/* Footer */}
      <View style={{ flexDirection: 'row' as any, alignItems: 'center' as any, justifyContent: 'center' as any, gap: 6, marginTop: 16 }}>
        <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: '#10B981' }} />
        <Text style={{ fontSize: 10, color: '#B0B8C4' }}>PDF · RGPD · Signé électroniquement</Text>
      </View>
    </View>
  );
}
